class Solution {
    ...
    public void dfs(Map<Integer, List<Integer>> graph, int current, int parent, int[] low, int[] disc, int time){
        disc[current] = low[current] = time++;
        for(Integer v : graph.get(current)){
            if(disc[v] == 0){
                dfs(graph, v, current, low, disc, time);
